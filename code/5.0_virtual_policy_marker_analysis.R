...
crs$`Housing in general` = !crs$Homelessness & 
  !crs$`Transitional housing` & 
  !crs$`Incremental housing` & 
  !crs$`Social housing` &
  !crs$`Market housing`
fwrite(crs, "large_output/crs_2024_update_housing_hfhi.csv")
...
ggsave(
  filename="output/virtual_category_Year.png",
  height=5,
  width=8
)
crs_by_category_wide = dcast(crs_by_category, Year~category_name, value.var="value")
fwrite(crs_by_category_wide, "output/virtual_category_Year.csv")
